...
class ScaffoldGenerator {
  ...
  async generate(options) {
    const {
      ...
      prefix,
      output,
      features = ['post-types', 'custom-login', 'theme', 'phpstan']
    } = options;
    ...
    // Use output directory directly (no subdirectory)
    ...
    console.log(chalk.white('   1. Activate the plugin in WordPress admin'));
    console.log(chalk.white('   2. Customize the code as needed'));
  }
  ...
  validateOptions(options) {
    ...
    if (!/^[a-zA-Z_]+$/.test(prefix)) {
      throw new Error('Prefix must contain only letters and underscores');
    }
  }
  ...
  prepareTemplateData(options) {
    ...
    const prefixLower = prefix.toLowerCase(); // For functions: msp_
    const prefixUpper = prefix.toUpperCase(); // For constants: MSP_
    ...
    return {
      ...
      // Code identifiers
      PREFIX: prefixLower,           // Lowercase for functions
      PREFIX_UPPER: prefixUpper,     // Uppercase for constants
      CONSTANT_NAME: constantName,   // Full name in uppercase
  ...
  async copyTemplateFiles(outputDir, templateData, features) {
    ...
    // PHPStan configuration files (optional feature)
    if (features.includes('phpstan')) {
      ...
      if (await fs.pathExists(composerSrc)) {
        await this.copyAndProcessFile(
          composerSrc,
          path.join(outputDir, 'composer.json'),
          templateData
        );
      } else {
      ...
      if (await fs.pathExists(phpstanConfigSrc)) {
        await this.copyAndProcessFile(
          phpstanConfigSrc,
          path.join(outputDir, 'phpstan.neon.dist'),
          templateData
        );
      } else {
